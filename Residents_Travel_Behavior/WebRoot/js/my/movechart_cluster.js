var xstep=80; // 移动步长，此参数越小，移动越平滑，最小值为1
var delay_time=2000; // 每步的时间间隔，此参数越小，移动速度越快
...
function tstart(){	//开始
	...
	if(FLAG == 1){	//开始主循环，以不断改变浮动层位置
		...
		if(YY <= 1840){
			...
			if(YY == 1840){
				mapget("div-charts1").style.display='block';//弹出统计框
				iframe1.window.location.href="charts/bar_cluster.html";//设定统计页面
				console.log("chart");
			}
